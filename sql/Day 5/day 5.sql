...
-- timestampdiff(hour, hire_date  ,now())as tenure_days 
-- from employees 
-- limit 10;
...
--   emp_id,
--   CONCAT(first_name, ' ', last_name) AS full_name,
--   TIMESTAMPDIFF(YEAR, hire_date, NOW()) AS years_with_company,
--   TIMESTAMPDIFF(MONTH, hire_date, NOW()) AS months_with_company,
--   
-- FROM employees
-- LIMIT 10;
...
--     -- Use modulo 12 to show the remaining months after full years are counted
...
-- lpad(emp_id,5,'0')as padded_email_id
-- from employees;
...
-- SELECT
--   emp_id,
--   CONCAT(LOWER(SUBSTRING(first_name,1,2)), LOWER(last_name)) AS username,
--   CONCAT(UPPER(first_name), ' ', UPPER(last_name)) AS display_name
-- FROM employees
-- LIMIT 10;
